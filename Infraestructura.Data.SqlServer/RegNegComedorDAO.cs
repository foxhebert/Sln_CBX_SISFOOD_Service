...
namespace Infraestructura.Data.SqlServer
{
    public class RegNegComedorDAO : Conexion
    {
        ...
        public List<ReglaNegocio> ListarRegNegCom(long intIdSesion, int intIdMenu, int intIdSoft, int intActivoFilter, string strfilter, int intfiltrojer, ref int intResult, ref string strMsjDB, ref string strMsjUsuario)
        {
            List<ReglaNegocio> lista = new List<ReglaNegocio>();
            using (SqlConnection cn = new SqlConnection(cadCnx))
            {
                SqlCommand cmd = new SqlCommand("TSP_TGREGLANEG_Q02", cn);//TSP_TGREGLANEG_COM_Q00
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        obj.strTipoConsu = reader.GetString(17);
                        obj.strContSub = reader.GetString(18);
            ...
            try
            {
                using (SqlConnection cn = new SqlConnection(cadCnx))
                {
                    ...
                    using (SqlTransaction trans = cn.BeginTransaction())
                    {
                        try
                        {
                            SqlCommand cmd = new SqlCommand("TSP_TGREGLANEG_IU01", cn);//TSP_TGREGLANEG_IU00
        ...
        //2.39
        public bool EliminarReglaNegocioCom(Session_Movi objSession, int intIdReglaNeg, ref int intResult, ref string strMsjDB, ref string strMsjUsuario)
        {
            bool exito = false;

            using (SqlConnection cn = new SqlConnection(cadCnx))
            {
                SqlCommand cmd = new SqlCommand("TSP_TGREGLANEG_D02", cn);//TSP_TGREGNEG_DET_D04>>TSP_TGREGLANEG_HE_DET_D04
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandTimeout = timeSQL;//21.04.2021
                cn.Open();
        ...
        //2.41 --Detalle de Regla
        public List<TGREGNEG_DET> ObtenerRegistroReglaNegocioDetCom(Session_Movi objSession, int intIdReglaNeg, ref int intResult, ref string strMsjDB, ref string strMsjUsuario)
        {
            ...
            using (SqlConnection cn = new SqlConnection(cadCnx))
            {
                ...
                Dictionary<string, object> param = new Dictionary<string, object>();
                param.Add("@intIdSesion", objSession.intIdSesion);
                param.Add("@intIdMenu", objSession.intIdMenu);//modificado 23.09.2021
                param.Add("@intIdSoft", objSession.intIdSoft);
                param.Add("@intIdReglaNeg", intIdReglaNeg);
                //salida
                param.Add("@intResult", 0);
            ...
            using (SqlConnection cn = new SqlConnection(cadCnx))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        obj.strCoRegNeg = reader.GetString(1);
                        obj.strDesRegNeg = reader.GetString(2);
                        obj.intextra2 = reader.GetInt32(3);//añadido 23.09.2021
                        obj.intextra1 = reader.GetInt32(4);//añadido 23.09.2021
                        obj.strRegNegCampo1 = reader.GetString(5);//añadido 23.09.2021
                        obj.strRegNegCampo2 = reader.GetString(6);//añadido 23.09.2021
                        obj.strRegNegCampo3 = reader.GetString(7);//añadido 23.09.2021
                        obj.strRegNegCampo4 = reader.GetString(8);//añadido 23.09.2021
                        obj.strRegNegCampo5 = reader.GetString(9);//añadido 23.09.2021
                        obj.bitFlActivo = reader.GetBoolean(10);
                        obj.bitFlPrincipal = reader.GetBoolean(11);
                        listDetCar.Add(obj);
                    }

...
namespace Infraestructura.Data.SqlServer
{
    public class UsuarioDAO : Conexion
    {
            ...
            using (SqlConnection cn = new SqlConnection(cadCnx))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        obj.intIdPersonal = reader.GetInt32(6);//añadido 26.07.2021
        ...
        //3.12 
        public List<TG_USUARIO> ValidarUsuario(long intIdSesion, int intIdMenu, int intIdSoft, string strusuario, string strcontraseña, string strCoSoft, string strIpHost, ref int Valida, ref int intResult, ref string strMsjDB, ref string strMsjUsuario)
        {
            List<TG_USUARIO> listaUsuario = new List<TG_USUARIO>();
            using (SqlConnection cn = new SqlConnection(cadCnx))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        if (!reader.IsDBNull(11))//Solo para Comedor
                        {
                            obj.intMarcadorTomaConsumo = reader.GetInt32(11);
                        }
                        listaUsuario.Add(obj);
                    }
                }
        ...
        //3.22
        public int CerrarSession(long intIdSesion, ref int intResult, ref string strMsjDB, ref string strMsjUsuario)
        {
            using (SqlConnection cn = new SqlConnection(cadCnx))
            {
                SqlCommand cmd = new SqlCommand("TSP_TSUSUARIO_SESIO_IU01", cn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandTimeout = timeSQL;
                cn.Open();

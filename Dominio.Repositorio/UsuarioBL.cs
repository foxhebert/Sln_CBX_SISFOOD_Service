...
namespace Dominio.Repositorio
{
    public class UsuarioBL
    {
        ...
        //3.10
        public bool IUsuario_T(long intIdSesion, int intIdMenu, int intIdSoft, TG_USUARIO objDatos, List<TSUSUAR_PERFI> listaDetallesUsuarioPerfil, List<TT_TSUSUAR_FILTRO> listaDetallesUsuarioFiltro, int intIdUsuario, int intTipoOperacion, ref string strMsjUsuario)
        {
            bool tudobem = false;
            int intResult = 0;
            string strMsjDB = "";
            ...
            try
                {
                    ...
                    if (intResult == 0)
                    {
                        if (!strMsjDB.Equals(""))
                        {
                            Log.AlmacenarLogMensaje("[IUsuario_T] => Respuesta del Procedimiento : " + strMsjDB);
                        Log.AlmacenarLogMensaje("[IUsuario_T] => Respuesta de la clase de Datos: " + Msj);
                        if (strMsjUsuario.Equals(""))
                                strMsjUsuario = strMsjDB;
                        }
                    }
            ...
            //        throw new Exception("Ocurrió un error de Transacción (IUsuario_T)");
            //}//Comentado 21.04.2021 solicitado por ER
                catch (SqlException ex)
                {
                    Log.AlmacenarLogError(ex, "UsuarioBL.cs: SqlException");
                    throw new Exception("Ocurrió un error en BD (IUsuario_T)");
                }
                catch (Exception ex)
                {
                    Log.AlmacenarLogError(ex, "UsuarioBL.cs: Exception");
                    throw new Exception("Error General (IUsuario_T)");
                }
        }
        //3.11
        ...
        public List<TG_USUARIO> ValidarUsuario(long intIdSesion, int intIdMenu, int intIdSoft, string strusuario, string strcontraseña,string strIpHost, string strCoSoft, ref int Valida, ref string strMsjUsuario)
        {
            ...
            try
            {
                ...
                {//añadido fin 05.07.2021
                    lista = objUsuario.ValidarUsuario(intIdSesion, 0, intIdSoft, strusuario, strcontraseña, strCoSoft, strIpHost, ref Valida, ref intResult, ref strMsjDB, ref strMsjUsuario);
                    if (intResult == 0)
                    {
                        if (!strMsjDB.Equals(""))
                        {
        ...
        //3.17
        public StringBuilder htmlMessageBody(CorreoEmp obj, string filtro)
        {
        ...
        //public List<TG_USUARIO> ValidarUsuario(long intIdSesion, int intIdMenu, int intIdSoft, string strusuario, string strcontraseña, string strCoSoft, ref int Valida, ref string strMsjUsuario)
        {
            ...
            try
            {
                ...
                string strMsjDB = "";
                intResult = objUsuario.CerrarSession(intIdSesion, ref intResult, ref strMsjDB, ref strMsjUsuario);
                if (intResult == 0)
                {
                    if (!strMsjDB.Equals(""))
                    {
                        Log.AlmacenarLogMensaje("[CerrarSession] => Respuesta del Procedimiento : " + strMsjDB);
                        if (strMsjUsuario.Equals(""))
                            strMsjUsuario = strMsjDB;
                    }
                }
            ...
            {
                Log.AlmacenarLogError(ex, "UsuarioBL.cs");
                throw new Exception("Ocurrió un error en BD (CerrarSession)");
            }

            catch (Exception ex)
            {
                Log.AlmacenarLogError(ex, "UsuarioBL.cs");
                throw new Exception("Error General (CerrarSession)");
            }
            return rpta;
        }
